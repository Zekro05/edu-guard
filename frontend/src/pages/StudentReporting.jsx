...
const ReportIncidentPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      formData.append("reporterType", "Student");
    ...
    <>
      <div>
        ...
        <div className="space-y-1">
          ...
          <Nav icon={<HelpCircle size={18} />} label="Get Support" path="/messages" />
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <section className={`rounded-2xl sm:rounded-3xl border p-5 sm:p-7 ${theme.card}`}>
              <div className="flex items-start gap-3 mb-5">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${darkMode ? "bg-purple-500/10 text-purple-400" : "bg-purple-50 text-purple-600"}`}>
                  <ShieldCheck size={19} />
                </div>
                <div>
                  <h3 className={`font-extrabold ${theme.heading}`}>Reporter Privacy</h3>
                  <p className={`text-xs mt-1 ${theme.body}`}>Choose whether your name should be shown as the reporter.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  ...
                  className={`text-left rounded-2xl border p-4 transition ${!isAnonymous ? (darkMode ? "border-green-600 bg-green-500/10" : "border-green-300 bg-green-50") : (darkMode ? "border-[#1A2C20] hover:bg-[#101F15]" : "border-gray-200 hover:bg-gray-50")}`}
                >
                  <p className={`text-sm font-bold ${theme.heading}`}>Submit with my identity</p>
                  <p className={`text-xs mt-1 ${theme.body}`}>Your name will be shown to authorized staff reviewing the report.</p>
                </button>

                <button
                  type="button"
                  onClick={() => setIsAnonymous(true)}
                  className={`text-left rounded-2xl border p-4 transition ${isAnonymous ? (darkMode ? "border-purple-600 bg-purple-500/10" : "border-purple-300 bg-purple-50") : (darkMode ? "border-[#1A2C20] hover:bg-[#101F15]" : "border-gray-200 hover:bg-gray-50")}`}
                >
                  <p className={`text-sm font-bold ${theme.heading}`}>Submit anonymously</p>
                  <p className={`text-xs mt-1 ${theme.body}`}>Your report will display the reporter as Anonymous.</p>
                </button>
              </div>
            </section>
